        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                if nextNode in notVisitedCorners:
                    #we remove the node from the list of the not visited corners if the node is actually a corner
                    notVisitedCorners.remove(nextNode)
                #create the new state of the pacman
...
#I could not make the implemented manhattan Heuristic work so i created my own <3
def myManhattanDistance(position, goal):
    xy1 = position
    xy2 = goal
    return ( (xy1[0] - xy2[0]) ** 2 + (xy1[1] - xy2[1]) ** 2 ) ** 0.5
...
def cornersHeuristic(state, problem):
    ...
    if (problem.isGoalState(state)):
        ...
        for corner in notVisitedCorners:
            ...
            possibleCornerDist.append(myManhattanDistance(currentNode,corner))
            currentNode = corner
